...
function isUnavailable(day) { return UNAVAILABLE.has(day); }

function rangeIsValid(a, b) {
  const lo = Math.min(a, b), hi = Math.max(a, b);
  for (let d = lo; d <= hi; d++) if (UNAVAILABLE.has(d)) return false;
  return true;
}
...
export default function HappyTailsCalendar() {
  ...
  const handleDay = (day) => {
    ...
    if (!start || (start && end)) {
      ...
      if (day === start) { setStart(null); }
      else if (rangeIsValid(start, day)) { setEnd(day); }
      ...
      <div className="mobile-frame">
        <div className="cal-screen">
          <header className="cal-header">
            <button className="cal-back" onClick={() => alert("Go back")}>←</button>
          ...
          <div className="cal-body">
            ...
            <p className="cal-instruction">
              {!start ? "Tap a start date"
               : !end  ? "Now tap an end date"
               : selectionLabel}
            </p>
          ...
          <div className="cal-footer">
            {start && (
              <button className="cal-clear-btn" onClick={() => { setStart(null); setEnd(null); }}>
                Clear
              </button>
            )}
            <button
              ...
              disabled={!end}
              onClick={() => alert(`Booking confirmed: ${selectionLabel}`)}
            >
              CONFIRM DATES →
            </button>
          </div>
